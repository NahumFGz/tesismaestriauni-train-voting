...
def crear_corpus_documentos(documentos: List[Dict[str, Any]]) -> List[str]:
    ...
    for doc in documentos:
        ...
        texto_completo = (
            ...
            f"{doc.get('sesion', '')} {doc.get('asunto', '')} "
            f"{doc.get('presidente', '')}"
        )
    ...
    def search(self, query: str, top_k: int = 10) -> List[str]:
        ...
        for idx in top_indices:
            if similarities[idx] > 0:  # Solo documentos con alguna similitud
                ...
                contexto = (
                    f"Votación del {doc.get('fecha_larga')}.\n"
                    f"Legislatura: {doc.get('legislatura')} – Congreso {doc.get('periodo_congreso')}.\n"
                    f"Periodo anual: {doc.get('periodo_anual')}.\n"
                    f"Asunto: {doc.get('asunto')}.\n"
                    f"Presidente de la sesión: {doc.get('presidente')}.\n"
                    f"URL: {doc.get('url')}"
                )
...
def main():
    ...
    print("🔄 Cargando datos de votaciones...")

    # Cargar documentos de votaciones
    try:
        with open(DATA_PATH, "r", encoding="utf-8") as f:
            documentos = json.load(f)
        print(f"✅ Cargados {len(documentos)} documentos de votaciones")
